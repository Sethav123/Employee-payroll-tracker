...
const getRandomEmployee = function(employeesArray) {
  const randomIndex = Math.floor(Math.random() * employeesArray.length); //uses math floor to round down the number generated by math.random and the length of the employees array 
  const randomEmployee = employeesArray[randomIndex]; //uses random index to select a random employee from the array
  console.log(`Random Employee: ${randomEmployee.firstName} ${randomEmployee.lastName} $${randomEmployee.salary}`); //console.logs the random employees data
}
